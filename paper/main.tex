...
\usepackage{url}
\usepackage{booktabs}
\title{Human Video Physical Intent Disambiguation: ICLR Main-Conference Gate Archive}
\author{Anonymous Authors}
\begin{document}
\maketitle
\begin{abstract}
This document is not an ICLR main-conference submission. It is the v3 main-conference gate for a generated robotics draft about learning robots from human video. The original research bet was: Separate human intent from morphology-specific motion artifacts in video demonstrations. After a stricter hostile review, the honest terminal decision is \textbf{KILL/ARCHIVE}. Submission-hardening version: v3.
\end{abstract}
...
\begin{itemize}
\item Estimating Articulated Human Motion with Covariance Scaled Sampling (2003)
\item CoRI: Communication of Robot Intent for Physical Human-Robot Interaction (2025)
\item Human motion intent learning based motion assistance control for a wearable exoskeleton (2018)
\item A Bio-Inspired Mechanism for Learning Robot Motion From Mirrored Human Demonstrations (2022)
\item A probabilistic model of human motion and navigation intent for mobile robot path planning (2009)
\end{itemize}
